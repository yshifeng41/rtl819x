...
void rtl865x_igmpLinkStatusChangeCallback(uint32 moduleIndex, rtl_igmpPortInfo_t * portInfo)
{
	...
	if(rtl_mCastModuleArray[moduleIndex].enableSnooping==TRUE)
	{
		...
		for(hashIndex=0;hashIndex<rtl_hashTableSize;hashIndex++)
			...
			while(groupEntryPtr!=NULL)
			{
				//delete br module group entry at the same time
				if(moduleIndex == nicIgmpModuleIndex)
				{
					memcpy(groupAddress, groupEntryPtr->groupAddr, 4);
				...
				{	
					...
					if(((1<<clientEntry->portNum) & portInfo->linkPortMask)==0)
					{
						...
						if(moduleIndex == nicIgmpModuleIndex && brGroupEntryPtr != NULL)
						{
							memcpy(clientAddress, clientEntry->clientAddr, 4);
							brClientEntryPtr = rtl_searchClientEntry(4,brGroupEntryPtr,0,clientAddress);
							if(brClientEntryPtr != NULL)
								rtl_deleteClientEntry(brGroupEntryPtr,brClientEntryPtr);
						}
						#ifdef CONFIG_RTK_VLAN_WAN_TAG_SUPPORT
						if(moduleIndex == nicIgmpModuleIndex && brGroupEntryPtr_2 != NULL)
						{
							memcpy(clientAddress, clientEntry->clientAddr, 4);
							brClientEntryPtr_2 = rtl_searchClientEntry(4,brGroupEntryPtr_2,0,clientAddress);
		...
		for(hashIndex=0;hashIndex<rtl_hashTableSize;hashIndex++)
			...
			while(groupEntryPtr!=NULL)
			{
				...
				if(moduleIndex == nicIgmpModuleIndex)
				{
					memcpy(groupAddress, groupEntryPtr->groupAddr, 16);
				...
				{	
					...
					if(((1<<clientEntry->portNum) & portInfo->linkPortMask)==0)
					{
						...
						if(moduleIndex == nicIgmpModuleIndex && brGroupEntryPtr != NULL)
						{
							memcpy(clientAddress, clientEntry->clientAddr, 16);
							brClientEntryPtr = rtl_searchClientEntry(6,brGroupEntryPtr,0,clientAddress);
							if(brClientEntryPtr != NULL)
								rtl_deleteClientEntry(brGroupEntryPtr,brClientEntryPtr);
						}
