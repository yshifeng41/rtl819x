...
void mp_set_rx_gain(struct rtl8192cd_priv *priv, unsigned char *data)
{
	...
	if (get_value_by_token((char *)data, "on")){
		...
		rx_gain_offset=priv->pmib->dot11RFEntry.pwrlevel5GHT40_1S_A[group_idx];
		initial_gain=mp_TransRxGainOffset2IG(rx_gain_offset);
		panic_printk("initial_gain_A %x\n",initial_gain);
		if ((initial_gain>0x28)||(initial_gain<0x1a)){
			panic_printk("Error initial_gain_A %x!!!\n",initial_gain);
			return;
		}
		RTL_W8(0xc50, (RTL_R8(0xc50)&0x80)|initial_gain);
		
		rx_gain_offset=priv->pmib->dot11RFEntry.pwrlevel5GHT40_1S_B[group_idx];
		initial_gain=mp_TransRxGainOffset2IG(rx_gain_offset);
		panic_printk("initial_gain_B %x\n",initial_gain);
		if ((initial_gain>0x28)||(initial_gain<0x1a)){
			panic_printk("Error initial_gain_B %x!!!\n",initial_gain);
			return;
		}
		RTL_W8(0xe50, (RTL_R8(0xe50)&0x80)|initial_gain);
		
		rx_gain_offset=priv->pmib->dot11RFEntry.pwrlevel5GHT40_1S_C[group_idx];
		initial_gain=mp_TransRxGainOffset2IG(rx_gain_offset);
		panic_printk("initial_gain_C %x\n",initial_gain);
		if ((initial_gain>0x28)||(initial_gain<0x1a)){
			panic_printk("Error initial_gain_C %x!!!\n",initial_gain);
			return;
		}
		RTL_W8(0x1850, (RTL_R8(0x1850)&0x80)|initial_gain);
		
		rx_gain_offset=priv->pmib->dot11RFEntry.pwrlevel5GHT40_1S_D[group_idx];
		initial_gain=mp_TransRxGainOffset2IG(rx_gain_offset);
		panic_printk("initial_gain_D %x\n",initial_gain);
		if ((initial_gain>0x28)||(initial_gain<0x1a)){
			panic_printk("Error initial_gain_D %x!!!\n",initial_gain);
			return;
		}
		RTL_W8(0x1a50, (RTL_R8(0x1a50)&0x80)|initial_gain);

		return;
	}else if (get_value_by_token((char *)data, "off")){
			
		RTL_W8(0xc50, (RTL_R8(0xc50)&0x80)|0x20);
		RTL_W8(0xe50, (RTL_R8(0xe50)&0x80)|0x20);	
		RTL_W8(0x1850, (RTL_R8(0x1850)&0x80)|0x20);
		RTL_W8(0x1a50, (RTL_R8(0x1a50)&0x80)|0x20);
...
void mp_pmac_tx(struct rtl8192cd_priv *priv, unsigned char *data)
{
	...
	if(PMacTxInfo.Mode == 2) /* Single Tone Tx */
		mpt_SetSingleTone_8814A(priv, TRUE, TRUE);
