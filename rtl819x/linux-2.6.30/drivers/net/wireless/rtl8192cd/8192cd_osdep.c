...
void check_txbf_support_cap(struct rtl8192cd_priv *priv)
{
	if(priv->pmib->dot11RFEntry.txbf)
	{
		...
		else if((get_rf_mimo_mode == MIMO_1T1R) || (get_rf_mimo_mode == MIMO_1T2R))
		{
...
int rtl8192cd_open(struct net_device *dev)
{
	...
	if(!strcmp(dev->name,"pwlan0"))
	{
		...
		wispWlanIndex=(passThruStatusWlan&WISP_WLAN_IDX_MASK)>>WISP_WLAN_IDX_RIGHT_SHIFT;
#else
		wispWlanIndex = 0;
#endif
		passThruWanIdx = wispWlanIndex;
#ifdef NETDEV_NO_PRIV
		((struct rtl8192cd_priv *)netdev_priv(dev))->wlan_priv = (struct rtl8192cd_priv *)wlan_device[wispWlanIndex].priv;
#else
		dev->priv = (struct rtl8192cd_priv *)wlan_device[wispWlanIndex].priv;
#endif
		((struct rtl8192cd_priv *)wlan_device[wispWlanIndex].priv)->pWlanDev=dev;	//pWlanDev point to the virtual pwlan0
		if(priv->drv_state&DRV_STATE_OPEN)
		{
