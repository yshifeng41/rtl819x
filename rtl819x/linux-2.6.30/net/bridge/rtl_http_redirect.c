...
static int is_required_url(struct sk_buff *skb)
{
	struct tcphdr otcph;
	int dataoff;
	struct iphdr *iph = ip_hdr(skb);
...
static int get_local_ip_mask(u_int32_t *ip,u_int32_t *mask)
{
...
unsigned int http_redirect_enter(struct sk_buff *skb)
{
	...
	if (iph->protocol == IPPROTO_TCP)
	{
		...
		if(is_required_url(skb))
		{
			DEBUGP("[%s:%d]Get required page!!!!!\n",__FUNCTION__,__LINE__);
			add_mac_to_list(mac);
			return 0;
		}
		if(get_local_ip_mask(&dev_ip,&dev_mask)==-1)
		{
			return -1;
		}
		DEBUGP("dev_ip:%x,dev_mask:%x\n",dev_ip,dev_mask);
		if(dev_ip != 0 && dev_mask !=0 &&((dev_ip & dev_mask) == (iph->daddr & dev_mask)))
		{/*local request*/
			DEBUGP("local request!!!\n");
			if(dev_ip == iph->daddr)/*just to this AP*/
			{
				return 0;
			}
			else/*to a LAN  server*/
			{
