/*
  ...
  *
  *	$Id: ev_tools.c,v 1.24 2011/02/14 08:13:53 brian Exp $
  */
...
static int do_cmd_event(int argc, char *argv[])
{
	int ret = 0, cmd_type, argNum;
      ...
      for (argNum = 1; argNum < argc; argNum++) {
          if (argv[argNum][0]=='-' && !strcmp(&argv[argNum][1], "do_id")) {
                cmd_type = strtol(argv[++argNum], (char **)NULL, 16);
                printf("do_id  cmd_type = 0x%x [%s]\n", cmd_type, argv[argNum]);
          } else if (argv[argNum][0]=='-' && !strcmp(&argv[argNum][1], "do_cmd")) {
                sprintf(tmpbuf, "%s", argv[++argNum]);
                printf("do_cmd  tmpbuf = %s \n", tmpbuf);
          } else if (argv[argNum][0]=='-' && !strcmp(&argv[argNum][1], "ssid")) {
                sprintf(tmpbuf, "%s", argv[++argNum]);
                printf("ssid  tmpbuf = %s \n", tmpbuf);
          } else if (argv[argNum][0]=='-' && !strcmp(&argv[argNum][1], "wpa_psk")) {
                sprintf(tmpbuf2, "%s", argv[++argNum]);
                printf("wpa_psk  tmpbuf2 = %s \n", tmpbuf2);
          } else if (argv[argNum][0]=='-' && !strcmp(&argv[argNum][1], "encrypted")) {
                cmd_type = strtol(argv[++argNum], (char **)NULL, 0);
                printf("encrypted  cmd_type = %d [%s]\n", cmd_type, argv[argNum]);
          }
      }

      if (argc > 5)
        ret = do_connect(tmpbuf, tmpbuf2, cmd_type);
      else
