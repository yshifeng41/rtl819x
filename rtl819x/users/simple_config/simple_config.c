		...
		if(1)//(pCtx->sc_wlan_status == 1)
		{
			...
			if (selret && FD_ISSET(sockfd_control, &fds) ) 
			{
				...
				{	
					...
					switch(pMsg->flag)
					{
						case SC_SAVE_PROFILE:
							if(accept_control == 1)
							{
								...
								if(res_msg.status != 0)
								{
									...
									accept_control = 0;
									//printf("save, accept_control is 0\n");
						...
						case SC_DEL_PROFILE:
							...
							if(accept_control == 1)
							{
								...
								if(res_msg.status != 0)
								{
									...
									accept_control = 0;
									//printf("del, accept_control is 0\n");
						...
						case SC_RENAME:
							...
							if(accept_control == 1)
							{
								...
								{
									accept_control = 0;
									//printf("rename, accept_control is 0\n");
									
									strcpy(pCtx->sc_device_name, pMsg->device_name);
									pCtx->sc_send_ack = SC_RENAME; // will send
									if(pCtx->sc_debug)
						...
						case SC_SUCCESS_ACK:
							if((pCtx->sc_send_ack == SC_SUCCESS_ACK) || (pCtx->sc_send_ack == SC_SUCCESS_IP))
							{
								if(pCtx->sc_ip_status != SC_DHCP_GETTING_IP) // config page
								{
									if(pCtx->sc_debug)
					...
					//  send control ack( even the operation is not done! )
					if((pCtx->sc_send_ack && (pCtx->sc_config_success==0)) || (res_msg.status== 0)) 
					{
						...
						if(pCtx->sc_debug == 2)
						{
							printf("control, sendto, pCtx->sc_send_ack is %d, res flag is %02x, res info is ",pCtx->sc_send_ack, res_msg.flag);
				...
				if(pCtx->sc_config_success == 1)
				{
					switch(pCtx->sc_send_ack)
					{
						...
						case SC_DEL_PROFILE:
							...
							if ( pCtx->sc_debug == 2 )
								printf("accept_control=1, then Remove_start.\n");
						...
						case SC_RENAME:
							if ( pCtx->sc_debug == 2 )
								printf("accept_control=1, then Rename_start.\n");
							...
							system(buf);
							if ( pCtx->sc_debug == 2 )
								printf("accept_control=1, then Rename_end.\n");
							break;
